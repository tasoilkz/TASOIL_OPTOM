...
# Установлена папка с фотографиями "перчатки"
    ...
    for item in items:
        ...
        if os.path.exists(img_path):
            try:
                ...
                # Конвертируем в RGB (убираем прозрачность для сохранения в JPEG)
                if img.mode in ("RGBA", "P"):
                    img = img.convert("RGB")
                
                # Оптимизация изображения: сжатие для уменьшения веса файла с сохранением чёткости
        ...
        # Отступ после картинки и разделитель
